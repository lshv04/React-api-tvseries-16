...
function Misterio({ networkCode }) {
  ...
  const { data, loading, error } = useFetch(
    `https://api.themoviedb.org/3/discover/tv?include_adult=false&include_null_first_air_dates=false&language=pt-BR&page=${page}&sort_by=popularity.desc&with_genres=9648&with_networks=${networkCode}`,
    options
  );
                ...
                <div className="card h-100 shadow border-0">
                  ...
                  <div className="card-body d-flex flex-column justify-content-between">
                    ...
                    <div>
                      ...
                      <Link to={`/${platform}/misterio/${item.id}`} className='detalhes'>Detalhes </Link>
                      </div>
                    </div>
                  </div>
                </div>
